...
class Multimodal_Dataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # q_img = self.image_processor(Image.new('RGB', (50, 50)))
        blank_img = Image.new('RGB', (50, 50))
        
        positive_img = self.cirr_dataset[item]["positive_value"]
        positive_img = [self.image_processor(self.img2pil(img.split("/")[-1])) if img is not "" else self.image_processor(blank_img) for img in positive_img]
        if not self.hn_mining:
            try:
                hn_images = random.sample(self.cirr_dataset[item]["hn_image"], self.train_group_size - 1)
            except Exception as e:
                # print(e)
                hn_images = self.cirr_dataset[item]["hn_image"]
        else:
        ...
        hn_images = [self.image_processor(self.img2pil(_hn.split("/")[-1])) if _hn is not "" else self.image_processor(blank_img) for _hn in hn_images]
        
        image_candidates = positive_img + hn_images    
        # return q_img, q_text, image_candidates
        return q_text, image_candidates
...
class Multimodal_Collator:
    ...
    def __call__(self, features):
        ...
        q_texts = [f[0] for f in features]
        image_candidates = [f[1] for f in features]
        ...
        return {"mm_it_query": q_text_collated, "image_candidate": c_image_collated}