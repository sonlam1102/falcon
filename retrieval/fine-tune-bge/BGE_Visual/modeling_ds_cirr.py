...
import sys
sys.path.append("/home/sonlt/drive/fact_check_end2end/FlagEmbedding/research/visual_bge/visual_bge")
...
class BGE_EVAToken(nn.Module):
    ...
    def forward(self, mm_it_query=None, image_candidate=None):
        ...
        query_reps = self.t_encoder(mm_it_query)
    ...
    def save(self, output_dir: str):
        state_dict = self.bge_model.state_dict()
        state_dict = type(state_dict)(
            {k: v.clone().cpu()
             for k,
                 v in state_dict.items()})
        self.bge_model.save_pretrained(output_dir, state_dict=state_dict)
